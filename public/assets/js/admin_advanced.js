...
async function loadPrograms() {
    try {
        ...
        const response = await fetch('api/admin_api.php?action=get_programs');
...
function updateProgramsTable() {
    ...
    tbody.innerHTML = programs.map(program => `
        <tr>
            ...
            <td>${program.total_sections || 0}</td>
            <td>
...
async function loadSections() {
    try {
        ...
        const response = await fetch(`api/program_management_api.php?action=get_sections${params.toString() ? '&' + params.toString() : ''}`);
