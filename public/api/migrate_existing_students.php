<?php
/**
 * Migrate Existing Students Script
 * Updates existing student records with roll number parsing data
 * Can be run manually or via admin panel
 */
...
// Authentication check for manual runs
$manual_run = isset($_GET['manual']) && $_GET['manual'] === 'true';
if ($manual_run) {
    if (!isLoggedIn() || !hasRole('admin')) {
        http_response_code(401);
        echo json_encode(['success' => false, 'error' => 'Authentication required for manual migration']);
        exit();
    }
}

try {
    // Get all students without admission year or with old data
    $stmt = $pdo->prepare("
