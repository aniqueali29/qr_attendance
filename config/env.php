<?php
// Simple environment loader for the QR Attendance project
...
function env_load_file(): void {
    ...
    $ENV_CACHE = [];
    $envPath = realpath(__DIR__ . '/../public/config.env');
    ...
    foreach ($lines as $line) {
        ...
        // Remove optional surrounding quotes without using PHP 8 functions
        if (strlen($val) >= 2) {
            $first = $val[0];
...
// Provide a helper to resolve storage path outside public web root
function storage_path(string $subPath = ''): string {
    $root = realpath(__DIR__ . '/..'); // project root dir
