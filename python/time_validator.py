...
class TimeValidator:
    ...
    def _load_timing_constants(self):
        ...
        # Convert to time objects with flexible parsing
        self.MORNING_CHECKIN_START = self._parse_time(morning_start)
        self.MORNING_CHECKIN_END = self._parse_time(morning_end)
        self.MORNING_CLASS_END = self._parse_time(morning_class_end)
        self.MORNING_CHECKOUT_START = self._parse_time(morning_checkout_start)
        self.MORNING_CHECKOUT_END = self._parse_time(morning_checkout_end)
        
        self.EVENING_CHECKIN_START = self._parse_time(evening_start)
        self.EVENING_CHECKIN_END = self._parse_time(evening_end)
        self.EVENING_CLASS_END = self._parse_time(evening_class_end)
        self.EVENING_CHECKOUT_START = self._parse_time(evening_checkout_start)
        self.EVENING_CHECKOUT_END = self._parse_time(evening_checkout_end)
        
        # Minimum duration from settings
        self.MINIMUM_DURATION_MINUTES = self.settings.get('minimum_duration_minutes', 120)
    
    def _parse_time(self, time_str: str) -> time:
        """Parse time string with flexible format support (12-hour, 24-hour, HH:MM, HH:MM:SS)."""
        try:
            ...
            try:
                ...
                try:
                    # Try HH:MM:SS format (24-hour)
                    return datetime.strptime(time_str, '%H:%M:%S').time()
                except ValueError:
                    try:
                        # Try HH:MM format (24-hour) and add seconds
                        return datetime.strptime(time_str, '%H:%M').time()
                    except ValueError:
                        # Fallback to default time
                        print(f"Warning: Invalid time format '{time_str}', using default 09:00:00")
                        return datetime.strptime('09:00:00', '%H:%M:%S').time()
    
    def _format_time_12h(self, time_obj: time) -> str:
    ...
    def validate_checkin_time(self, student_id: str, current_time: Optional[datetime] = None, 
        ...
        return {
            ...
            'shift': shift,
            'current_time': current_time.isoformat(),
            'checkin_start': self._format_time_12h(timings['checkin_start']),
            'checkin_end': self._format_time_12h(timings['checkin_end']),
            'class_end': timings['class_end'].strftime('%I:%M %p'),
            'is_within_window': is_within_window,
            'time_until_close': time_until_close,
            'error': None if is_within_window else f"Check-in not allowed. Window: {self._format_time_12h(timings['checkin_start'])} - {self._format_time_12h(timings['checkin_end'])}"
        }
    ...
    def validate_checkout_time(self, student_id: str, current_time: Optional[datetime] = None, 
        ...
        return {
            ...
            'shift': shift,
            'current_time': current_time.isoformat(),
            'checkout_start': self._format_time_12h(timings['checkout_start']),
            'checkout_end': self._format_time_12h(timings['checkout_end']),
            'class_end': timings['class_end'].strftime('%I:%M %p'),
            'is_within_window': is_within_window,
            'time_until_close': time_until_close,
            'error': None if is_within_window else f"Check-out not allowed. Window: {self._format_time_12h(timings['checkout_start'])} - {self._format_time_12h(timings['checkout_end'])}"
        }
    ...
    def get_next_checkin_window(self, shift: str, current_time: Optional[datetime] = None) -> Dict:
        ...
        if timings['checkin_start'] <= current_time_only <= timings['checkin_end']:
            return {
                ...
                'window_start': timings['checkin_start'].strftime('%I:%M %p'),
                'window_end': timings['checkin_end'].strftime('%I:%M %p'),
    ...
    def get_shift_schedule(self, shift: str) -> Dict:
        ...
        return {
            ...
            'schedule': {
                'checkin_window': {
                    'start': timings['checkin_start'].strftime('%I:%M %p'),
                    'end': timings['checkin_end'].strftime('%I:%M %p'),
                    'duration_hours': timings['checkin_window_hours']
                },
                'checkout_window': {
                    'start': timings['checkout_start'].strftime('%I:%M %p'),
                    'end': timings['checkout_end'].strftime('%I:%M %p')
                },
                'class_session': {
                    'start': timings['checkin_start'].strftime('%I:%M %p'),
                    'end': timings['class_end'].strftime('%I:%M %p'),
            ...
            'description': f"{shift} shift: Check-in {timings['checkin_start'].strftime('%I:%M %p')}-{timings['checkin_end'].strftime('%I:%M %p')}, Check-out {timings['checkout_start'].strftime('%I:%M %p')}-{timings['checkout_end'].strftime('%I:%M %p')}, Class until {timings['class_end'].strftime('%I:%M %p')}"
        }
